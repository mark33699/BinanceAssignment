//
//  UIColor+Ext.swift
//  BinanceAssignment
//
//  Created by 謝飛飛 on 2019/12/13.
//  Copyright © 2019 MarkFly. All rights reserved.
//

import UIKit

extension UIColor
{
    static let generalBackground = UIColor.init(named: "GeneralBackground")
    static let orderTextGreen = UIColor.init(named: "OrderTextGreen")
    static let orderTextRed = UIColor.init(named: "OrderTextRed")
    static let orderBackgroundGreen = UIColor.init(named: "OrderBackgroundGreen")
    static let orderBackgroundRed = UIColor.init(named: "OrderBackgroundRed")
}
